...
- `malloc`, `calloc`, `realloc`, `aligned_alloc` → `free`. Check every path.
- `strdup`, `strndup`, `asprintf` → `free`. Often forgotten.
- `fopen` → `fclose`. `fdopen` → `fclose`. `open` → `close`. `socket` → `close`.
- `mmap` → `munmap`. `shm_open` → `shm_unlink` + `close`.
