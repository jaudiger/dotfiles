...
name: code-audit
description: >
  Audit source files for bugs. Run /code-audit to see available languages and
  concerns. Each concern applies a targeted checklist to the given targets
...
| `diff:branch` | Detect default branch, then `git diff <default>...HEAD` |
...
| `rust` | leaks, uaf, deadlocks, races, oom, lifecycle, overflow, error-handling, async-bugs, ub, injection |
| `go` | leaks, deadlocks, races, lifecycle, overflow, error-handling, async-bugs, injection |
| `java` | leaks, deadlocks, races, lifecycle, overflow, error-handling, injection |
