...
5. **Constant replacement** — for each literal constant in the source, consider: if it were changed to a different value (0 → 1, "" → "x", timeout from 30 to 0), would any test fail? Flag constants that appear in logic-critical positions but whose specific values are never tested.
