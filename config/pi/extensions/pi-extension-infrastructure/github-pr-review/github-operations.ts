...
type RepositoryResolutionRequest = {
...
/**
 * Provider policy and wording used by the provider-neutral GitHub operations.
 * Repository resolution intentionally remains a hook: providers may use
 * different URL parsing and checkout safety rules.
 */
export type GithubPrReviewProvider = {
  search: (cwd: string) => Promise<{
    args: string[];
    cwd: string;
    repository?: string;
  }>;
  ...
  resolveRemote: (cwd: string, repository: string) => Promise<string>;
  /** Extract a repository from each result when search spans repositories. */
  repositoryFromUrl?: (value: unknown) => string | undefined;
  authorRules: (value: Json) => boolean;
  candidateEligibility: (value: Json) => boolean;
  queueBehavior: {
    repository: (repository: string) => QueueRepository | undefined;
  };
  labels: {
    subject: string;
  ...
  messages: {
    ...
    noCandidate: string;
    notEligible: (pullRequest: string) => string;
    noLongerEligible?: string;
    invalidNumber?: string;
    statusChecksFailure?: (pullRequest: number, detail: string) => string;
    diffFailure: (pullRequest: number, detail: string) => string;
    invalidDiff?: (pullRequest: number) => string;
    checkoutConfirmation: string;
    checkoutSuccess: (pullRequest: number, head: string) => string;
  };
  policy?: {
    validateDiff?: (
      diff: string,
    ...
    supersedeTemporaryDirectoryPrefix?: string;
    pullRequestViewArgs?: (reference: string, fields: string) => string[];
    validateMutationRepository?: (review: MutationTarget) => Promise<void>;
    mutationEligibility?: (value: Json) => boolean;
    validateWorkingDirectory?: (cwd: string) => Promise<string>;
  };
};

const defaultPullRequestMessages = {
  invalidUrl: "Pull request had no valid GitHub URL.",
  invalidRepository: "Pull request had no valid GitHub repository.",
  noCandidate: "No eligible open GitHub pull request was found.",
  notEligible: (pullRequest: string) =>
    `PR ${pullRequest} is not an eligible open pull request.`,
  noLongerEligible: "The pull request is no longer eligible for this provider.",
  invalidNumber: "GitHub pull request had no valid number.",
  statusChecksFailure: (pullRequest: number, detail: string) =>
    `Could not retrieve status checks for PR ${pullRequest}${detail ? `: ${detail}` : "."}`,
  diffFailure: (pullRequest: number, detail: string) =>
    `Could not retrieve the pull request diff for PR ${pullRequest} (${detail}).`,
  checkoutConfirmation: "Checkout pull request?",
  checkoutSuccess: (pullRequest: number, head: string) =>
    `Checked out reviewed PR ${pullRequest} at ${head}.`,
};
...
function messages(
  provider: GithubPrReviewProvider,
): GithubPrReviewProvider["messages"] {
  return { ...defaultPullRequestMessages, ...provider.messages };
}

function pullRequestViewArgs(
  provider: GithubPrReviewProvider,
  reference: string,
  fields: string,
): string[] {
  return (
    provider.policy?.pullRequestViewArgs?.(reference, fields) || [
      "pr",
      "view",
      reference,
      "--json",
...
function eligible(provider: GithubPrReviewProvider, value: Json): boolean {
  return provider.candidateEligibility(value) && provider.authorRules(value);
}

function mutationEligible(
  provider: GithubPrReviewProvider,
  value: Json,
): boolean {
  return (
    provider.policy?.mutationEligibility?.(value) ?? eligible(provider, value)
  );
}
...
async function fetchMergeQueueHistory(
  ...
  const queueRepository = provider.queueBehavior.repository(repository);
  if (!queueRepository)
    return {
      removals: [],
      workflowRuns: [],
  ...
  const { owner, name } = queueRepository;
  const removals: Json[] = [];
  ...
  try {
    for (;;) {
      ...
      const nodes = Array.isArray(object(timeline).nodes)
        ? (object(timeline).nodes as unknown[])
        : [];
      ...
      const pageInfo = object(object(timeline).pageInfo);
...
export async function listCandidates(
  ...
  const search = await provider.search(cwd);
  const listed = await ghJson(search.args, search.cwd);
  const candidates = Array.isArray(listed) ? listed : [];
  return candidates.flatMap((item) => {
    if (!eligible(provider, item)) return [];
    const pr = prNumber(item.number);
    const url = string(item.url);
    const repository = provider.repositoryFromUrl?.(url) || search.repository;
    if (!pr || !url || (provider.repositoryFromUrl && !repository)) return [];
    return [
      {
        number: pr,
...
export async function fetchReviewDiff(
  ...
  const resolved = await provider.resolveRepository({ candidate, cwd });
  const diff = await capture(
    "gh",
    ["pr", "diff", String(candidate.number), "--repo", resolved.repository],
    resolved.cwd,
  );
  if (diff.exitCode !== 0 || !diff.output) {
    const detail = diff.output.trim().slice(-2000);
    throw new Error(
      provider.messages.diffFailure(
        candidate.number,
        `exit code ${diff.exitCode}${detail ? `: ${detail}` : ""}`,
      ),
    );
  }
  await provider.policy?.validateDiff?.(diff.output, candidate);
  return diff.output;
}
...
export async function prepareReview(
  ...
  const prefix =
    provider.policy?.temporaryDirectoryPrefix || "pi-github-pr-review-";
  const directory = await mkdtemp(join(tmpdir(), prefix));
  try {
    const reviewCwd =
      (await provider.policy?.validateWorkingDirectory?.(cwd)) || cwd;
    const requested = requestedPullRequest?.trim();
    let metadata: Json | undefined;
    if (requested) {
      metadata = object(
        await ghJson(
          pullRequestViewArgs(provider, requested, pullRequestFields),
          reviewCwd,
        ),
      );
    } else {
      const candidate = (await listCandidates(provider, reviewCwd))[0];
      if (candidate)
        metadata = object(
          await ghJson(
            pullRequestViewArgs(provider, candidate.url, pullRequestFields),
            reviewCwd,
          ),
        );
    }
    const providerMessages = messages(provider);
    if (!metadata || !eligible(provider, metadata))
      throw new Error(
        requested
          ? providerMessages.notEligible(requested)
          : providerMessages.noCandidate,
      );
    const pr = prNumber(metadata.number);
    if (!pr)
      throw new Error(
        providerMessages.invalidNumber ||
          "GitHub pull request had no valid number.",
      );
    ...
    if (!candidate.url) throw new Error(providerMessages.invalidUrl);
    const diff = await fetchReviewDiff(provider, candidate, resolved.cwd);
    ...
    if (checksJson.exitCode !== 0) {
      ...
      throw new Error(
        providerMessages.statusChecksFailure?.(pr, detail) ||
          `Could not retrieve status checks for PR ${pr}${detail ? `: ${detail}` : "."}`,
      );
    }
    ...
    const enriched = {
      ...
      repositoryWorkspace: resolved.cwd,
    };
    await writeFile(
    ...
    await writeFile(join(directory, "diff.patch"), diff, { mode: 0o600 });
    return {
      directory,
      number: pr,
      repository: resolved.repository,
      metadata: enriched,
      snapshot: snapshotFromMetadata(metadata, resolved.repository),
      cwd: resolved.cwd,
      sessionId,
    };
  } catch (error) {
...
export async function prepareMutationTarget(
  ...
  if (!eligible(provider, metadata))
    throw new Error(messages(provider).notEligible(String(candidate.number)));
...
async function refreshedMetadata(
  ...
  for (let attempt = 0; ; attempt += 1) {
    await provider.policy?.validateMutationRepository?.(review);
    ...
    if (!mutationEligible(provider, metadata))
      throw new Error(
        messages(provider).noLongerEligible ||
          "The pull request is no longer eligible for this provider.",
      );
...
export async function refreshReview(
  ...
  return refreshedMetadata(provider, review, requireMergeable, progress);
}
...
async function observeMergeOutcome(
  ...
  for (let attempt = 0; ; attempt += 1) {
    const state = await refreshReviewState(provider, review);
...
async function mergePreparedReview(
  ...
  const [metadata, checks] = await Promise.all([
    refreshedMetadata(provider, review, true, progress),
  ...
  try {
    ...
    if (!approved) {
      ...
      approved = true;
      state = await refreshReview(provider, review, true, progress);
      assertReviewedHead(review, state, expectedHead);
  ...
  try {
    ...
    const outcome = await observeMergeOutcome(
      provider,
      review,
      expectedHead,
      progress,
...
export async function mergeReview(
  ...
  if (
    ...
    !(await ctx.ui.confirm(
      reviews.length === 1
        ? `Merge ${provider.labels.subject} pull request?`
        : `Merge ${provider.labels.subjectPlural} pull requests?`,
...
async function supersedeSource(
  ...
  try {
    review.snapshot = await refreshReviewState(provider, review);
  } catch (error) {
    failures.push(
...
export async function supersedeReview(
  ...
  for (let index = 0; index < reviews.length; index += 1) {
    ...
    const metadata = await refreshedMetadata(provider, review);
  ...
  for (let index = 0; index < initial.length; index += 1) {
    ...
    const metadata = await refreshedMetadata(provider, source.review);
  ...
  const workspace = await mkdtemp(
    join(
      tmpdir(),
      provider.policy?.supersedeTemporaryDirectoryPrefix ||
        `${provider.policy?.temporaryDirectoryPrefix || "pi-github-pr-review-"}supersede-`,
    ),
  );
...
async function checkoutSingleReview(
  ...
  const initialMetadata = await refreshedMetadata(provider, review);
  ...
  if (
    !ctx.hasUI ||
    !(await ctx.ui.confirm(
      provider.messages.checkoutConfirmation,
  ...
  const remote = await provider.resolveRemote(review.cwd, review.repository);
  await successful(
    "git",
  ...
  return provider.messages.checkoutSuccess(review.number, currentHead);
}
...
export async function checkoutReview(
  ...
  return checkoutSingleReview(provider, reviews[0]!, ctx, progress);
}
