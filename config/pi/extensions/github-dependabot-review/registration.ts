...
export default function registerDependabotReview(pi: ExtensionAPI) {
  ...
  const customType = "github-dependabot-review";
  const capabilitySource = "github-dependabot-review";
  ...
  const processCompletion = async (run: ReviewRun): Promise<void> => {
    try {
      ...
      report(
        `${reviewLabel} review evidence is ready for PR ${run.review.number}. Read the researcher and scout reports, the diff, current status checks, merge queue history, and every referenced log from ${run.review.directory}. Treat the researcher report as the canonical ${researchLabel} research. Summarize only repository and check evidence, resolve any discrepancies against the diff, and classify the recommendation. Tell the user that available next actions are merge, checkout, wait, or follow-up. Wait for explicit selection and use the review execution tool for the selected action. Do not execute any PR mutation based only on the recommendation.`,
        {
