...
use ($SCRIPT_DIR | path join "handler-nixpkgs-review.nu")
use ($SCRIPT_DIR | path join "handler-nl.nu")
...
use ($SCRIPT_DIR | path join "handler-rg.nu")
...
export def dispatcher [argv: list<string>]: nothing -> record<decision: string, reason: string> {
    ...
    match ($argv | get 0?) {
        ...
        "nixpkgs-review" => (handler-nixpkgs-review handler $argv),
        "nl" => (handler-nl handler $argv),
